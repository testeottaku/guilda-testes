...
// --- Ajustes (Firestore) ----------------------------------------------------
// Coleção: ajustes / Doc: membros / Campo: tag
// O documento só é criado quando um Líder salvar a tag.
export async function getMemberTagConfig() {
  try {
    const snap = await getDoc(doc(db, "ajustes", "membros"));
    if (!snap.exists()) return null;
    const data = snap.data() || {};
    const tag = (data.tag || "").toString().trim();
    return tag ? tag : null;
  } catch (e) {
    console.error("Erro ao ler tag (ajustes/membros):", e);
    return null;
  }
}
...
export async function setMemberTagConfig(tag) {
  ...
  await setDoc(
    doc(db, "ajustes", "membros"),
    { tag: clean, updatedAt: serverTimestamp() },
    { merge: true }
  );
  return true;
}
